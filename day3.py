...
directions1 = input1.split(",")
directions2 = input2.split(",")

Line = namedtuple("line", "x1 y1 x2 y2 xmin xmax ymin ymax startdelay")


def intersect(l1: Line, l2: Line) -> Optional[Tuple]:
    if (
        l1.x1 == l1.x2
        and l2.y1 == l2.y2
        and l2.xmin <= l1.x1 <= l2.xmax
        and l1.ymin <= l2.y1 <= l1.ymax
    ):
        return (l1.x1, l2.y1)
    elif (
        l1.y1 == l1.y2
        and l2.x1 == l2.x2
        and l1.xmin <= l2.x1 <= l1.xmax
        and l2.ymin <= l1.y1 <= l2.ymax
    ):
        return (l2.x1, l1.y1)
...
def parse_direction(direction: str, x1: int, y1: int, delay: int) -> Line:
    ...
    return Line(x1, y1, x2, y2, xmin, xmax, ymin, ymax, delay)


def parse_input(directions: List[str]):
    wire: List[Line] = []
    last_x, last_y, delay = 0, 0, 0
    for direction in directions:
        line = parse_direction(direction, last_x, last_y, delay)
        wire.append(line)
        last_x, last_y = line.x2, line.y2
        delay += int(direction[1:])
    return wire


wire1 = parse_input(directions1)
wire2 = parse_input(directions2)


intersections: Dict[Tuple[int, int], int] = {}
for l1, l2 in product(wire1, wire2):
    if intersection := intersect(l1, l2):
        ix, iy = intersection
        steps1 = l1.startdelay + abs(ix - l1.x1) + abs(iy - l1.y1)
        steps2 = l2.startdelay + abs(ix - l2.x1) + abs(iy - l2.y1)
        intersections[intersection] = steps1 + steps2

print(min(abs(a) + abs(b) for a, b in intersections if (a != 0 or b != 0)))
print(min(v for v in intersections.values() if v != 0))
