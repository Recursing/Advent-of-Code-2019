...
while registers["running"]:
    ip = registers["ip"]
    mode, opcode = mem[ip] // 100, mem[ip] % 100
    op, arg_num = operations[opcode]
    modes = list(map(int, reversed(f"{mode:0>5}")))

    args = mem[ip + 1 : ip + arg_num + 1]
    registers["ip"] += arg_num + 1
