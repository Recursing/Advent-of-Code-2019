...
def step():
    ...
    old_color = canvas[current_pos]
    new_color = next(proc_gen)
    assert new_color == "input"
    new_color = proc_gen.send(old_color)
    while new_color == "input":
        new_color = proc_gen.send(old_color)
        print(f"{new_color=}")
    assert new_color in (0, 1)
    canvas[current_pos] = new_color
    rotation = proc_gen.send(new_color)
    while rotation == "input":
        rotation = proc_gen.send(new_color)
        print(f"{rotation=}")
    ...
    if rotation == 0:
        ...
        direction *= 1j
    pos[0] += int(direction.real)
    pos[1] += int(direction.imag)
...
minx = min(canvas)[0]
maxx = max(canvas)[0]
miny = min(y for x, y in canvas)
maxy = max(y for x, y in canvas)
